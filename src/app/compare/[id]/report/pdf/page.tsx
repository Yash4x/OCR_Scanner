...
export default async function ComparisonReportPdfPage({
  params,
}: {
  ...
  return (
    <main className="min-h-screen bg-white px-6 py-8 text-slate-900 print:px-0 print:py-0">
      <div className="mx-auto mb-4 max-w-5xl print:hidden">
        <h1 className="text-lg font-semibold">Comparison Report PDF Export</h1>
        <p className="mt-1 text-sm text-slate-600">
          Use your browser print dialog to Save as PDF (Ctrl+P / Cmd+P).
        </p>
      </div>
      <article className="mx-auto max-w-5xl rounded-lg border border-slate-200 p-6 print:border-0 print:p-0">
        <pre className="whitespace-pre-wrap break-words font-mono text-sm leading-6">{markdown}</pre>
      </article>
    </main>
  );
}
